...
export default {
  ...
  methods: {
    ...mapActions(dataStore, ['setEditObj']),
    search() {
      let searchObj = {
        name: this.quizList.name,
        start_date: this.quizList.startDate,
        end_date: this.quizList.endDate,
      ...
      if (this.quizList.startDate > this.quizList.endDate) {
        alert("開始日期不能晚於結束日期");
        return;
      }
    },
    ...
    deleteQuiz() {
      ...
      if (this.deleteCheckbox.length > 0 > 0) {
...
<template>
  ...
  <div class="search">
    ...
    <input class="titlename input" type="text" />
    <h6 class="time que">統計時間：</h6>
    <input class="start input" type="date" />
    <h6 class="to que">到</h6>
    <input class="end input" type="date" />
    <button class="searchbtn" @click="this.search()">搜尋</button>
  </div>
  <div class="list">
    ...
    <table class="questionaire">
      <thead>
        ...
        <th scope="col" class="thead" style="width: 300px;">問卷名稱</th>
        <th scope="col" class="thead" style="width: 100px;">狀態</th>
        <th scope="col" class="thead" style="width: 160px;">開始時間</th>
        <th scope="col" class="thead" style="width: 160px;">結束時間</th>
      ...
      <tbody>
        <tr class="content" v-for="(item, index) in this.calDataWithStatus">
          <td>{{ item.id }}</td>
          <td>{{ item.name }}</td>
          <td><div class="statusBg" :class="getStatusClass(item.status)">
              {{ item.status }}
            </div></td>
