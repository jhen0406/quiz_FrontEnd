...
<template>
  ...
  <div class="search">
    ...
    <input class="start input" type="date" v-model="this.quizList.startDate" />
    <h6 class="to que">到</h6>
    <input class="end input" type="date" v-model="this.quizList.endDate" />
    ...
    <table class="questionaire">
      ...
      <tbody>
        <tr class="content" v-for="(item, index) in this.calDataWithStatus">
          <td>{{ item.id }}</td>
          <td>
            <RouterLink to="/FrontFillin" style="background-color: transparent;" >
              <button class="questionTitleBtn" @click="this.bringToInput(index)" 
              :disabled="item.status === '已結束' || item.status === '尚未開始'">{{ item.name }}</button></RouterLink>
          </td>
          <td>
            <div class="statusBg" :class="getStatusClass(item.status)">
              {{ item.status }}
            </div>
          </td>
          ...
          <td>
            <button
              class="edit"
              :disabled="item.status === '尚未開始' || item.status === '開放中'"
            >
              <i
